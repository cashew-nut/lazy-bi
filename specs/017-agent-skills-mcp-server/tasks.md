...
- [ ] T013 [US1] Promote `app/api/chat.py`'s private orchestration helpers — `_start_ask`, `_handle_decision`, `_persist_learned`, `_resolved_query_dict`, `_summarize` — into public functions on `app/nlq.py` (`start_ask`, `handle_decision`, `persist_learned`, `resolved_query_dict`, `summarize`), matching `app/nlq.py`'s own module docstring, which already declares itself the intended reuse seam for a second caller (research.md R6). Update `app/api/chat.py`'s routes to call the promoted `app/nlq.py` functions instead of owning the logic — behavior unchanged, rename + move only.
- [ ] T014 [US1] Regression-check `tests/test_chat_api.py` against the T013 promotion — existing chat endpoints behave identically (depends on: T013)
- [ ] T015 [US1] Create `app/skills_analytics.py` with the `ask_question` skill: input `{question: str, conversation_id?: int}`; handler auto-creates a conversation via `registry.conversation_store.create()` when `conversation_id` is omitted (or 404-equivalents on an id the caller doesn't own), then calls the promoted `nlq.start_ask`/`nlq.handle_decision`; `min_role="viewer"`, `rate_limited=True`; output always includes `conversation_id` (contracts/mcp-server.md) (depends on: T003, T013)
...
- [ ] T018 [P] [US1] End-to-end `ask_question` tests through the mounted `/mcp` app in `tests/test_mcp_server.py`: an answerable question returns `outcome: "answered"` with real data; an ambiguous question returns `outcome: "clarification"` naming real candidates; an out-of-scope question returns `outcome: "declined"`; a follow-up call reusing the returned `conversation_id` gets follow-up context; each call is audited (`mcp_skill:ask_question`) (depends on: T016, T017)
...
- [ ] T024 [US3] `tests/test_agents.py`: using a fixture `agents/*.yaml` (not the shipped `analytics.yaml`), remove a skill from its declared list, reload via `app/agents.py`'s loader + `Registry`, and assert the MCP server's exposed tool set (`tools/list`) changes accordingly with no code change (depends on: T009, T020)
...
- **User Story 3 (Phase 5)**: depends on Foundational + at least one real skill existing to prove reconfiguration against (T020, from US2) — functionally independent of both prior stories' *behavior*, only reuses their artifacts as fixtures
...
- Once Foundational is done, US1 and the `app/skills_analytics.py`-independent parts of Setup/Polish could staff in parallel, but US2/US3 each need one artifact from the story before them (T015, T020) — plan sequential P1 → P2 → P3 delivery unless staffed with care around that one shared file
