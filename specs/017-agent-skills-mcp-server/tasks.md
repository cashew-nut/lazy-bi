...
- [X] T020 [US2] Add the `list_models` skill to `app/skills_analytics.py`: input `{}`; handler wraps `nlq.build_catalog()` and serializes the resulting `ModelCatalogEntry` list to JSON; `min_role="viewer"`, `rate_limited=False` (depends on: T003, T015)
- [X] T021 [US2] Add `list_models` to `agents/analytics.yaml`'s `skills` list (depends on: T016, T020)
- [X] T022 [US2] Implement role-filtered `tools/list` in `app/mcpserver.py`: an `on_list_tools` middleware hook drops any tool whose skill's `min_role` the connection's authenticated role doesn't satisfy (research.md R7) (depends on: T008) — implemented as part of the Foundational `AgentExposureMiddleware` (T008), which is fully dynamic; dedicated T023 tests below prove it for this story
...
- [X] T025 [US3] Document the Agent/Skill/MCP framework in `README.md`: the `agents/*.yaml` declaration format, the two shipped skills (`ask_question`, `list_models`), how an external client connects to `/mcp` and authenticates, and `CI_MCP_RATE_LIMIT_PER_MIN` — per the constitution's "update README as part of the feature, not a follow-up" (depends on: T016, T021)
...
- [X] T026 [P] Add module docstrings to `app/skills.py`, `app/agents.py`, `app/mcpserver.py`, `app/skills_analytics.py` explaining each module's role and referencing this spec's key decisions (research.md R1-R7), matching this repo's existing module-docstring convention (e.g. `app/nlq.py`, `app/llm.py`)
- [X] T027 Run the full suite (`.venv/bin/python -m pytest tests/`) and confirm zero regressions across the whole app, not just the new tests — 920 passed, 0 failed (two earlier single-test failures in an unrelated file, each a different test, each reproducing 0/1 in isolation, confirmed transient/environmental, not a regression)
- [X] T028 Run `quickstart.md` end-to-end against the running app with a real MCP client connection — Constitution Principle IV, adapted per plan.md. Ran a real `uvicorn` instance + the real `fastmcp.Client` library (not the test suite's TestClient) over genuine HTTP: `tools/list` returns exactly `[ask_question, list_models]`; `list_models` returns all 7 real demo models; `ask_question` makes a real network call to the real Anthropic API (which 401s on the placeholder key) and the `TranslatorError` path converts it to a clean `outcome: "error"` result, not a hang or an unhandled exception; an unauthenticated connection is rejected (401) before any MCP protocol exchange. Steps requiring a real working LLM key (4-7) are covered by the automated FakeTranslator-based suite instead, matching spec 012's own established precedent.
