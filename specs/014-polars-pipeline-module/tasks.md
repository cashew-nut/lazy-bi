...
- [ ] T024 [US3] Post-run lineage sync in `app/pipeline_jobs.py`: after a successful run, validate declarations against the runner-reported `output_schema`, persist it on the run row (`finish_run`'s `output_schema` param, T004) and record `lineage_ok`/`lineage_issues` (non-blocking, FR-018), then regenerate the matched model's section via `replace_lineage_yaml` + hot-reload (depends on T022, T023). Orphan-marking is a *write* triggered by the DELETE request itself, not a reload side effect: in `DELETE /pipelines/{name}` (T011), after removing the pipeline file, look up its matched model (if any) and call `replace_lineage_yaml` with `orphaned=true` before returning — never defer this to `reload_all()`.
