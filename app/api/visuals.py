...
def _validate_visual_spec(spec: dict) -> None:
    ...
    for m in query.get("inline_measures") or []:
        expr = m.get("expr") or ""
        unknown = sqlgrammar.referenced_parameter_names(expr) - seen
        if unknown:
            raise HTTPException(
                status_code=400,
                detail=f"measure '{m.get('name')}': references undeclared parameter(s) {sorted(unknown)}",
            )
        # LAG()'s offset argument requires an int-typed parameter — this one
        ...
        for pname in sqlgrammar.lag_period_param_names(expr):
            if declared_types.get(pname) != "int":
                raise HTTPException(
                    status_code=400,
                    detail=f"measure '{m.get('name')}': LAG()'s offset argument references parameter "
